...
prompts = []
acc_ids = []
model_data_string_db = model_data[
    model_data.gene_name.isin(string_db_data.gene_name)
].copy()
model_data_string_db = model_data_string_db[
    model_data_string_db.gene_name.notnull()
].copy()
model_data_string_db.set_index("gene_name", inplace=True)
big_data_genes = model_data_string_db.drop(columns=["top_1000_sig", "top_500_sig"])
gene_count = 0
...
out_fname = f"prompts_string_db_{gene_count}_deg_{str(datetime.now()).replace(' ','_')}"
prompt_df.to_csv(
    f"{out_fname}.csv",
    index=False,
)
